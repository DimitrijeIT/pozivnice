...
/**
 * Generate Passport Layout Preview
 * Travel document style wedding invitation with boarding pass RSVP
 */
...
const THEME_FONTS = {
  classic: `<link href="https://fonts.googleapis.com/css2?family=Cinzel:wght@400;500;600&family=Source+Sans+3:wght@400;500;600&family=JetBrains+Mono:wght@400;500&display=swap" rel="stylesheet">`,
  tropical: `<link href="https://fonts.googleapis.com/css2?family=Cinzel:wght@400;500;600&family=Source+Sans+3:wght@400;500;600&family=JetBrains+Mono:wght@400;500&display=swap" rel="stylesheet">`
};
...
function generateThemedPage(data, theme) {
  const baseTemplate = fs.readFileSync(path.join(TEMPLATES_DIR, 'base-passport.html'), 'utf8');
...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    :root{--primary:#1B3A5F;--primary-dark:#0F2540;--bg:#E8E4DC;--white:#fff;--border:#ddd;--muted:#666}
    body{font-family:'Inter',sans-serif;background:var(--bg);color:#1a1a1a;line-height:1.6;min-height:100vh}
    .container{max-width:900px;margin:0 auto;padding:0 1rem}
    .header{background:var(--primary);color:#C9A962;padding:1.5rem 0;position:sticky;top:0;z-index:100}
    .header-content{display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:1rem}
    .header-title{font-size:1.25rem;font-weight:600}
    .expiry-timer{display:flex;align-items:center;gap:0.5rem;padding:0.5rem 1rem;background:rgba(201,169,98,0.2);border-radius:8px;font-size:0.875rem}
    .main{padding:3rem 0}
    ...
    .badge{display:inline-block;background:var(--primary);color:#C9A962;padding:0.25rem 0.75rem;border-radius:50px;font-size:0.75rem;margin-bottom:1rem}
    .theme-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(350px,1fr));gap:2rem}
    .theme-card{background:var(--white);border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.1);transition:transform 0.3s}
    ...
    .btn-primary{background:var(--primary);color:#C9A962;flex:1}
...
<body>
  <header class="header"><div class="container"><div class="header-content">
    <h1 class="header-title">✈️ ${data.bride_name} & ${data.groom_name}</h1>
  ...
  <main class="main"><div class="container">
    <div class="intro">
      <span class="badge">✈️ Пасош</span>
      <h1>Изаберите тему позивнице</h1>
      <p>Путна исправа за ваше венчање — пасош стил са укрцајном картом.</p>
    </div>
...
async function generatePreview(slug) {
  console.log(`\n✈️ Generating passport layout for: ${slug}\n`);

  const data = loadWeddingData(slug);
  data.slug = slug;

  const outputDir = path.join(PREVIEW_DIR, `${slug}-passport`);
  ...
  console.log(`\n✅ Done! URL: http://localhost:${config.DEV_SERVER_PORT}/preview/${slug}-passport/\n`);
}
...
module.exports = { generatePassportPreview: generatePreview };
