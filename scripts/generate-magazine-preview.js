...
/**
 * Generate Magazine Layout Preview
 * Vogue-style editorial wedding invitation
 */
...
const THEME_FONTS = {
  vogue: `<link href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,700;1,400&family=Source+Sans+3:wght@400;500;600&family=Cormorant+Garamond:ital,wght@0,400;0,500;1,400&display=swap" rel="stylesheet">`,
  glossy: `<link href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,700;1,400&family=Source+Sans+3:wght@400;500;600&family=Cormorant+Garamond:ital,wght@0,400;0,500;1,400&display=swap" rel="stylesheet">`
};
...
function replacePlaceholders(html, data) {
  ...
  if (data.story_text) {
    ...
    if (pullQuote.length > 80) pullQuote = pullQuote.substring(0, 80) + '...';
  }
...
function generateThemedPage(data, theme) {
  const baseTemplate = fs.readFileSync(path.join(TEMPLATES_DIR, 'base-magazine.html'), 'utf8');
...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    :root{--primary:#C41E3A;--primary-dark:#9A1830;--bg:#F5F5F5;--white:#fff;--border:#ddd;--muted:#666}
...
<body>
  ...
  <main class="main"><div class="container">
    <div class="intro">
      <span class="badge">📖 Магазин</span>
      <h1>Изаберите тему позивнице</h1>
      <p>Едиторијал о вашој љубави — магазин стил са насловном страном.</p>
    </div>
...
async function generatePreview(slug) {
  console.log(`\n📖 Generating magazine layout for: ${slug}\n`);

  const data = loadWeddingData(slug);
  data.slug = slug;

  const outputDir = path.join(PREVIEW_DIR, `${slug}-magazine`);
  ...
  console.log(`\n✅ Done! URL: http://localhost:${config.DEV_SERVER_PORT}/preview/${slug}-magazine/\n`);
}
...
module.exports = { generateMagazinePreview: generatePreview };
