...
/**
 * Generate Old Money Estate Layout Preview
 * Quiet luxury with monogram crest and classical elegance
 */
...
const THEME_DESCRIPTIONS = {
  estate: 'Класична морнарска плава са златним акцентима',
  ivory: 'Топли кремасти тонови са суптилном елеганцијом'
};

const THEME_FONTS = {
  estate: `<link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,500;0,600;1,400&family=EB+Garamond:ital,wght@0,400;0,500;1,400&family=Courier+Prime&display=swap" rel="stylesheet">`,
  ivory: `<link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,500;0,600;1,400&family=EB+Garamond:ital,wght@0,400;0,500;1,400&family=Courier+Prime&display=swap" rel="stylesheet">`
};
...
function replacePlaceholders(html, data) {
  const brideInitial = (data.bride_name || 'A').charAt(0).toUpperCase();
  const groomInitial = (data.groom_name || 'B').charAt(0).toUpperCase();

  const replacements = {
    'BRIDE_NAME': data.bride_name || '',
    'GROOM_NAME': data.groom_name || '',
    'BRIDE_NAME_INITIAL': brideInitial,
    'GROOM_NAME_INITIAL': groomInitial,
...
function generateThemedPage(data, theme) {
  const baseTemplate = fs.readFileSync(path.join(TEMPLATES_DIR, 'base-oldmoney.html'), 'utf8');
...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    :root{--primary:#0A2239;--primary-dark:#071828;--bg:#FFFFF0;--white:#fff;--border:#D4D0C8;--muted:#666}
    body{font-family:'Inter',sans-serif;background:var(--bg);color:#1C1C1C;line-height:1.6;min-height:100vh}
    ...
    .expiry-timer{display:flex;align-items:center;gap:0.5rem;padding:0.5rem 1rem;background:rgba(10,34,57,0.05);border-radius:8px;font-size:0.875rem;color:var(--primary)}
    .main{padding:3rem 0}
...
<body>
  ...
  <main class="main"><div class="container">
    <div class="intro">
      <span class="badge">👑 Класика</span>
      <h1>Изаберите тему позивнице</h1>
      <p>Тиха елеганција — монограм, класична типографија и безвременски стил.</p>
    </div>
...
async function generatePreview(slug) {
  console.log(`\n👑 Generating old money estate layout for: ${slug}\n`);

  const data = loadWeddingData(slug);
  data.slug = slug;

  const outputDir = path.join(PREVIEW_DIR, `${slug}-oldmoney`);
  ...
  console.log(`\n✅ Done! URL: http://localhost:${config.DEV_SERVER_PORT}/preview/${slug}-oldmoney/\n`);
}
...
module.exports = { generateOldMoneyPreview: generatePreview };
