...
/**
 * Generate Wabi-Sabi Scroll Layout Preview
 * Organic imperfection with Japanese aesthetic influence
 */
...
const THEMES_DIR = path.join(TEMPLATES_DIR, 'themes-wabisabi');
...
const THEME_DESCRIPTIONS = {
  paper: 'Топли тонови папира са природном несавршеношћу',
  ink: 'Дубоки угљени тонови попут суми-е мастила'
};

const THEME_FONTS = {
  paper: `<link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,500;1,400&family=Crimson+Text:ital,wght@0,400;1,400&display=swap" rel="stylesheet">`,
  ink: `<link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,500;1,400&family=Crimson+Text:ital,wght@0,400;1,400&display=swap" rel="stylesheet">`
};
...
function generateThemedPage(data, theme) {
  const baseTemplate = fs.readFileSync(path.join(TEMPLATES_DIR, 'base-wabisabi.html'), 'utf8');
...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    :root{--primary:#8B7355;--primary-dark:#6B5545;--bg:#F5F0E8;--white:#FAF7F2;--border:#E0DCD5;--muted:#6B6560}
    body{font-family:'Inter',sans-serif;background:var(--bg);color:#3D3D3D;line-height:1.6;min-height:100vh}
    ...
    .expiry-timer{display:flex;align-items:center;gap:0.5rem;padding:0.5rem 1rem;background:rgba(139,115,85,0.1);border-radius:8px;font-size:0.875rem;color:var(--primary)}
    .main{padding:3rem 0}
    ...
    .theme-card{background:var(--white);border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.06);transition:transform 0.3s}
...
<body>
  ...
  <main class="main"><div class="container">
    <div class="intro">
      <span class="badge">🍃 Ваби-саби</span>
      <h1>Изаберите тему позивнице</h1>
      <p>Лепота несавршености — органски облици, природне текстуре и медитативни ритам.</p>
    </div>
...
async function generatePreview(slug) {
  console.log(`\n🍃 Generating wabi-sabi scroll layout for: ${slug}\n`);

  const data = loadWeddingData(slug);
  data.slug = slug;

  const outputDir = path.join(PREVIEW_DIR, `${slug}-wabisabi`);
  ...
  console.log(`\n✅ Done! URL: http://localhost:${config.DEV_SERVER_PORT}/preview/${slug}-wabisabi/\n`);
}
...
module.exports = { generateWabiSabiPreview: generatePreview };
