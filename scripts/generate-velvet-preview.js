...
/**
 * Generate Midnight Velvet Layout Preview
 * Dark maximalist design with jewel tones and baroque elements
 */
...
const THEMES_DIR = path.join(TEMPLATES_DIR, 'themes-velvet');
const PREVIEW_DIR = path.join(ROOT_DIR, config.PATHS.preview);
const DATA_DIR = path.join(ROOT_DIR, config.PATHS.data);

const THEMES = ['burgundy', 'navy'];

const THEME_NAMES = {
  burgundy: 'Бордо кадифа',
  navy: 'Поноћно плава'
};

const THEME_DESCRIPTIONS = {
  burgundy: 'Дубоке боје вина са златним акцентима',
  navy: 'Елегантна поноћна плава са луксузним детаљима'
};

const THEME_FONTS = {
  burgundy: `<link href="https://fonts.googleapis.com/css2?family=Cinzel+Decorative:wght@400;700&family=Cormorant+Garamond:ital,wght@0,400;0,500;1,400&display=swap" rel="stylesheet">`,
  navy: `<link href="https://fonts.googleapis.com/css2?family=Cinzel+Decorative:wght@400;700&family=Cormorant+Garamond:ital,wght@0,400;0,500;1,400&display=swap" rel="stylesheet">`
};
...
function replacePlaceholders(html, data) {
  ...
  const replacements = {
    ...
    'INVITATION_INTRO': data.invitation_intro || 'Са задовољством вас позивају',
...
function generateThemedPage(data, theme) {
  const baseTemplate = fs.readFileSync(path.join(TEMPLATES_DIR, 'base-velvet.html'), 'utf8');
...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    :root{--primary:#D4AF37;--primary-dark:#B8960B;--bg:#1A0A0F;--surface:#2D1620;--text:#F5E6E8;--border:rgba(212,175,55,0.2);--muted:#B39EA3}
    ...
    .expiry-timer{display:flex;align-items:center;gap:0.5rem;padding:0.5rem 1rem;background:rgba(212,175,55,0.1);border-radius:8px;font-size:0.875rem;color:var(--primary)}
    .main{padding:3rem 0}
...
<body>
  ...
  <main class="main"><div class="container">
    <div class="intro">
      <span class="badge">🌙 Кадифа</span>
      <h1>Изаберите тему позивнице</h1>
      <p>Поноћна елеганција — луксузни дизајн са барокним детаљима и златним акцентима.</p>
    </div>
...
async function generatePreview(slug) {
  console.log(`\n🌙 Generating midnight velvet layout for: ${slug}\n`);

  const data = loadWeddingData(slug);
  data.slug = slug;

  const outputDir = path.join(PREVIEW_DIR, `${slug}-velvet`);
  ...
  console.log(`\n✅ Done! URL: http://localhost:${config.DEV_SERVER_PORT}/preview/${slug}-velvet/\n`);
}
...
module.exports = { generateVelvetPreview: generatePreview };
