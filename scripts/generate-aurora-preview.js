...
/**
 * Generate Aurora Layout Preview
 * Northern lights with cosmic particle effects
 */
...
const THEME_FONTS = {
  northern: `<link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@200;300;400&family=Inter:wght@300;400&display=swap" rel="stylesheet">`,
  cosmic: `<link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@200;300;400&family=Inter:wght@300;400&display=swap" rel="stylesheet">`
};
...
function replacePlaceholders(html, data) {
  const replacements = {
    ...
    'INVITATION_INTRO': data.invitation_intro || 'Под звездама вас позивамо',
...
function generateThemedPage(data, theme) {
  const baseTemplate = fs.readFileSync(path.join(TEMPLATES_DIR, 'base-aurora.html'), 'utf8');
...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@200;300;400&family=Inter:wght@400;500;600&display=swap" rel="stylesheet">
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    :root{--primary:#9B4DCA;--primary-dark:#7B3DA0;--bg:#0B0B1A;--surface:rgba(20,20,40,0.9);--white:#E0E0E0;--border:rgba(155,77,202,0.3);--muted:#8888AA;--aurora:#00FF88}
    body{font-family:'Inter',sans-serif;background:var(--bg);color:var(--white);line-height:1.6;min-height:100vh;position:relative;overflow-x:hidden}
    body::before{content:'';position:fixed;top:0;left:0;width:100%;height:100%;background:linear-gradient(180deg,transparent 0%,rgba(0,255,136,0.05) 30%,rgba(155,77,202,0.08) 60%,transparent 100%);pointer-events:none;z-index:0}
    .container{max-width:900px;margin:0 auto;padding:0 1rem;position:relative;z-index:1}
    .header{background:var(--surface);backdrop-filter:blur(10px);border-bottom:1px solid var(--border);padding:1.5rem 0;position:sticky;top:0;z-index:100}
    .header-content{display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:1rem}
    .header-title{font-family:'Montserrat',sans-serif;font-weight:200;font-size:1.25rem;letter-spacing:0.1em}
    .header-title span{background:linear-gradient(135deg,var(--aurora),var(--primary));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
    .expiry-timer{display:flex;align-items:center;gap:0.5rem;padding:0.5rem 1rem;background:rgba(155,77,202,0.2);border-radius:8px;font-size:0.875rem;color:var(--aurora)}
    .main{padding:3rem 0}
    .intro{text-align:center;margin-bottom:3rem}
    .intro h1{font-family:'Montserrat',sans-serif;font-weight:200;font-size:2rem;letter-spacing:0.15em;margin-bottom:0.75rem}
    .intro p{color:var(--muted);max-width:600px;margin:0 auto}
    .badge{display:inline-block;background:linear-gradient(135deg,var(--primary),var(--aurora));color:var(--bg);padding:0.25rem 0.75rem;border-radius:50px;font-size:0.75rem;margin-bottom:1rem}
    .theme-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(350px,1fr));gap:2rem}
    .theme-card{background:var(--surface);backdrop-filter:blur(10px);border:1px solid var(--border);border-radius:16px;overflow:hidden;transition:transform 0.3s}
    ...
    .theme-name{font-family:'Montserrat',sans-serif;font-weight:300;font-size:1.25rem;margin-bottom:0.5rem}
    ...
    .btn-primary{background:linear-gradient(135deg,var(--primary),var(--aurora));color:var(--white);flex:1}
    .btn-primary:hover{transform:translateY(-2px);box-shadow:0 5px 20px rgba(155,77,202,0.4)}
    .btn-secondary{background:transparent;color:var(--white);border:1px solid var(--border)}
    .btn-secondary:hover{border-color:var(--aurora)}
    .success-message{display:none;text-align:center;padding:4rem 2rem}
    .success-message.active{display:block}
    .success-message h2{font-family:'Montserrat',sans-serif;font-weight:200;letter-spacing:0.1em;background:linear-gradient(135deg,var(--aurora),var(--primary));-webkit-background-clip:text;-webkit-text-fill-color:transparent}
    ...
    .modal{background:var(--surface);backdrop-filter:blur(20px);border:1px solid var(--border);border-radius:16px;max-width:400px;padding:2rem;text-align:center}
    .modal h2{font-family:'Montserrat',sans-serif;font-weight:300;margin-bottom:0.75rem}
...
<body>
  ...
  <main class="main"><div class="container">
    <div class="intro">
      <span class="badge">✨ Аурора</span>
      <h1>Изаберите тему позивнице</h1>
      <p>Северна светла — космичка романса под звездама и плесом аурора боjа.</p>
    </div>
...
async function generatePreview(slug) {
  console.log(`\n✨ Generating aurora layout for: ${slug}\n`);

  const data = loadWeddingData(slug);
  data.slug = slug;

  const outputDir = path.join(PREVIEW_DIR, `${slug}-aurora`);
  ...
  console.log(`\n✅ Done! URL: http://localhost:${config.DEV_SERVER_PORT}/preview/${slug}-aurora/\n`);
}
...
module.exports = { generateAuroraPreview: generatePreview };
