...
/**
 * Generate Mediterranean Tiles Layout Preview
 * Sun-soaked coastal design with tile patterns
 */
...
const THEME_FONTS = {
  amalfi: `<link href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,500;1,400&family=Libre+Franklin:wght@300;400;500&display=swap" rel="stylesheet">`,
  santorini: `<link href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,500;1,400&family=Libre+Franklin:wght@300;400;500&display=swap" rel="stylesheet">`
};
...
function generateThemedPage(data, theme) {
  const baseTemplate = fs.readFileSync(path.join(TEMPLATES_DIR, 'base-mediterranean.html'), 'utf8');
...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    :root{--primary:#0047AB;--primary-dark:#003380;--bg:#FFFEF7;--white:#FFF;--border:#D4D0C8;--muted:#5A7A94;--terracotta:#C2452D}
    body{font-family:'Inter',sans-serif;background:var(--bg);color:#1C3D5C;line-height:1.6;min-height:100vh}
    .container{max-width:900px;margin:0 auto;padding:0 1rem}
    .header{background:var(--white);border-bottom:3px solid var(--primary);padding:1.5rem 0;position:sticky;top:0;z-index:100}
    .header-content{display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:1rem}
    .header-title{font-size:1.25rem;font-weight:600}
    .header-title span{color:var(--primary)}
    .expiry-timer{display:flex;align-items:center;gap:0.5rem;padding:0.5rem 1rem;background:rgba(0,71,171,0.1);border-radius:8px;font-size:0.875rem;color:var(--primary)}
    .main{padding:3rem 0}
    ...
    .theme-card{background:var(--white);border:3px solid var(--primary);border-radius:12px;overflow:hidden;transition:transform 0.3s}
    ...
    .theme-name{font-size:1.25rem;font-weight:600;margin-bottom:0.5rem;color:var(--primary)}
    ...
    .modal{background:var(--white);border:3px solid var(--primary);border-radius:12px;max-width:400px;padding:2rem;text-align:center}
    .modal h2{margin-bottom:0.75rem;color:var(--primary)}
...
<body>
  ...
  <main class="main"><div class="container">
    <div class="intro">
      <span class="badge">🍋 Медитеран</span>
      <h1>Изаберите тему позивнице</h1>
      <p>Сунчана обала — керамички мотиви, топле боје и мирис лимуна са Амалфија.</p>
    </div>
...
async function generatePreview(slug) {
  console.log(`\n🍋 Generating mediterranean tiles layout for: ${slug}\n`);

  const data = loadWeddingData(slug);
  data.slug = slug;

  const outputDir = path.join(PREVIEW_DIR, `${slug}-mediterranean`);
  ...
  console.log(`\n✅ Done! URL: http://localhost:${config.DEV_SERVER_PORT}/preview/${slug}-mediterranean/\n`);
}
...
module.exports = { generateMediterraneanPreview: generatePreview };
