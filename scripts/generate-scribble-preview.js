...
/**
 * Generate Scribble Love Layout Preview
 * Hand-drawn playful design with notebook aesthetic
 */
...
const THEME_DESCRIPTIONS = {
  watercolor: 'Меки акварел тонови са разигранном енергијом',
  pencil: 'Скицирани стил оловке на крафт папиру'
};

const THEME_FONTS = {
  watercolor: `<link href="https://fonts.googleapis.com/css2?family=Caveat:wght@400;700&family=Patrick+Hand&display=swap" rel="stylesheet">`,
  pencil: `<link href="https://fonts.googleapis.com/css2?family=Caveat:wght@400;700&family=Patrick+Hand&display=swap" rel="stylesheet">`
};
...
function replacePlaceholders(html, data) {
  const replacements = {
    ...
    'INVITATION_INTRO': data.invitation_intro || 'Драги наши,',
...
function generateThemedPage(data, theme) {
  const baseTemplate = fs.readFileSync(path.join(TEMPLATES_DIR, 'base-scribble.html'), 'utf8');
...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    :root{--primary:#E8B4B8;--primary-dark:#D4A0A4;--bg:#FFFEF7;--white:#FFF;--border:#DDD;--muted:#666;--line:#333}
    body{font-family:'Inter',sans-serif;background:var(--bg);color:#1A1A1A;line-height:1.6;min-height:100vh}
    .container{max-width:900px;margin:0 auto;padding:0 1rem}
    .header{background:var(--white);border-bottom:3px solid var(--line);padding:1.5rem 0;position:sticky;top:0;z-index:100}
    .header-content{display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:1rem}
    .header-title{font-family:'Caveat',cursive;font-size:1.5rem}
    .header-title span{color:var(--primary)}
    .expiry-timer{display:flex;align-items:center;gap:0.5rem;padding:0.5rem 1rem;background:var(--primary);border-radius:20px;font-size:0.875rem;color:var(--line)}
    .main{padding:3rem 0}
    .intro{text-align:center;margin-bottom:3rem}
    .intro h1{font-family:'Caveat',cursive;font-size:2.5rem;margin-bottom:0.75rem}
    .intro p{color:var(--muted);max-width:600px;margin:0 auto}
    .badge{display:inline-block;background:var(--line);color:var(--bg);padding:0.25rem 0.75rem;border-radius:50px;font-size:0.75rem;margin-bottom:1rem}
    .theme-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(350px,1fr));gap:2rem}
    .theme-card{background:var(--white);border:3px solid var(--line);border-radius:12px;overflow:hidden;box-shadow:4px 4px 0 var(--line);transition:transform 0.2s}
    .theme-card:hover{transform:translate(-2px,-2px);box-shadow:6px 6px 0 var(--line)}
    .theme-preview{aspect-ratio:3/4;overflow:hidden;background:var(--bg)}
    .theme-info{padding:1.5rem}
    .theme-name{font-family:'Caveat',cursive;font-size:1.5rem;margin-bottom:0.5rem}
    .theme-description{color:var(--muted);margin-bottom:1.25rem;font-size:0.9rem}
    .theme-actions{display:flex;gap:0.75rem}
    .btn{padding:0.75rem 1.25rem;border-radius:8px;font-size:0.9rem;font-weight:500;cursor:pointer;text-decoration:none;border:2px solid var(--line);font-family:inherit;transition:all 0.2s}
    .btn-primary{background:var(--primary);color:var(--line);flex:1}
    .btn-primary:hover{background:var(--primary-dark)}
    .btn-secondary{background:transparent;color:var(--line)}
    .btn-secondary:hover{background:var(--bg)}
    .success-message{display:none;text-align:center;padding:4rem 2rem}
    .success-message.active{display:block}
    .success-message h2{font-family:'Caveat',cursive;font-size:2rem}
    ...
    .modal{background:var(--white);border:3px solid var(--line);border-radius:12px;max-width:400px;padding:2rem;text-align:center;box-shadow:6px 6px 0 var(--line)}
    .modal h2{font-family:'Caveat',cursive;font-size:1.8rem;margin-bottom:0.75rem}
...
<body>
  ...
  <main class="main"><div class="container">
    <div class="intro">
      <span class="badge">💕 Скица</span>
      <h1>Изаберите тему позивнице</h1>
      <p>Разиграни дизајн — руком цртане илустрације, весели детаљи и бележница љубави.</p>
    </div>
...
async function generatePreview(slug) {
  console.log(`\n💕 Generating scribble love layout for: ${slug}\n`);

  const data = loadWeddingData(slug);
  data.slug = slug;

  const outputDir = path.join(PREVIEW_DIR, `${slug}-scribble`);
  ...
  console.log(`\n✅ Done! URL: http://localhost:${config.DEV_SERVER_PORT}/preview/${slug}-scribble/\n`);
}
...
module.exports = { generateScribblePreview: generatePreview };
