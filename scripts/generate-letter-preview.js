...
/**
 * Generate Love Letter Layout Preview
 * Romantic calligraphy style with envelope and wax seal
 */
...
const THEME_FONTS = {
  romantic: `<link href="https://fonts.googleapis.com/css2?family=Great+Vibes&family=Cormorant+Garamond:ital,wght@0,400;0,500;1,400&display=swap" rel="stylesheet">`,
  vintage: `<link href="https://fonts.googleapis.com/css2?family=Great+Vibes&family=Cormorant+Garamond:ital,wght@0,400;0,500;1,400&display=swap" rel="stylesheet">`
};
...
function generateThemedPage(data, theme) {
  const baseTemplate = fs.readFileSync(path.join(TEMPLATES_DIR, 'base-letter.html'), 'utf8');
...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    :root{--primary:#B76E79;--primary-dark:#8B4D57;--bg:#F8E8E0;--white:#FFFEF7;--border:#D4A574;--muted:#8B7355}
    body{font-family:'Inter',sans-serif;background:var(--bg);color:#4A3728;line-height:1.6;min-height:100vh}
    ...
    .expiry-timer{display:flex;align-items:center;gap:0.5rem;padding:0.5rem 1rem;background:rgba(183,110,121,0.1);border:1px solid rgba(183,110,121,0.3);border-radius:8px;font-size:0.875rem;color:var(--primary)}
    .main{padding:3rem 0}
...
<body>
  ...
  <main class="main"><div class="container">
    <div class="intro">
      <span class="badge">💌 Писмо</span>
      <h1>Изаберите тему позивнице</h1>
      <p>Љубавно писмо са воштаним печатом — романтична калиграфија.</p>
    </div>
...
async function generatePreview(slug) {
  console.log(`\n💌 Generating love letter layout for: ${slug}\n`);

  const data = loadWeddingData(slug);
  data.slug = slug;

  const outputDir = path.join(PREVIEW_DIR, `${slug}-letter`);
  ...
  console.log(`\n✅ Done! URL: http://localhost:${config.DEV_SERVER_PORT}/preview/${slug}-letter/\n`);
}
...
module.exports = { generateLetterPreview: generatePreview };
