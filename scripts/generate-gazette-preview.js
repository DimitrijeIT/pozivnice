...
/**
 * Generate Gazette Layout Preview
 *
 * Newspaper-style wedding announcement - all info on one page
 * with editorial layout, headlines, columns, and sidebar.
 */
...
const THEME_FONTS = {
  broadsheet: `<link href="https://fonts.googleapis.com/css2?family=Libre+Baskerville:ital,wght@0,400;0,700;1,400&family=Playfair+Display:wght@400;700&display=swap" rel="stylesheet">`,
  tabloid: `<link href="https://fonts.googleapis.com/css2?family=Anton&family=Source+Sans+3:wght@400;600;700&display=swap" rel="stylesheet">`
};
...
function replacePlaceholders(html, data) {
  // Extract a pull quote from story text
  let pullQuote = '';
  if (data.story_text) {
...
function generateThemedPage(data, theme) {
  const baseTemplate = fs.readFileSync(path.join(TEMPLATES_DIR, 'base-gazette.html'), 'utf8');
...
function generateIndexPage(data) {
  ...
  const themeCards = GAZETTE_THEMES.map(theme => `
...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    :root{--primary:#1a1a1a;--primary-dark:#000;--bg:#E8E4DC;--white:#fff;--border:#ddd;--muted:#666}
    body{font-family:'Inter',sans-serif;background:var(--bg);color:var(--primary);line-height:1.6;min-height:100vh}
    ...
    .expiry-timer{display:flex;align-items:center;gap:0.5rem;padding:0.5rem 1rem;background:#FEF3C7;border-radius:8px;font-size:0.875rem}
    .main{padding:3rem 0}
    ...
    .theme-card{background:var(--white);border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.1);transition:transform 0.3s,box-shadow 0.3s}
...
<body>
  ...
  <main class="main" id="main-content">
    <div class="container">
      <div class="intro">
        <span class="badge">📰 Новински стил</span>
        <h1>Изаберите тему позивнице</h1>
        <p>Све информације на једној страници у стилу новина — класични или модерни таблоид.</p>
      </div>
...
async function generateGazettePreview(slug) {
  console.log(`\n📰 Generating gazette layout for: ${slug}\n`);

  const data = loadWeddingData(slug);
  data.slug = slug;

  const outputDir = path.join(PREVIEW_DIR, `${slug}-gazette`);
  await fs.ensureDir(outputDir);

  console.log(`📁 Output: ${outputDir}\n`);

  for (const theme of GAZETTE_THEMES) {
  ...
  console.log(`\n✅ Done! URL: http://localhost:${config.DEV_SERVER_PORT}/preview/${slug}-gazette/\n`);
}

if (require.main === module) {
  generateGazettePreview(process.argv[2] || 'demo').catch(console.error);
}

module.exports = { generateGazettePreview };
