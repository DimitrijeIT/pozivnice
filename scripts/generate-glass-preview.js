...
/**
 * Generate Glass Gallery Layout Preview
 * Glassmorphism with floating panels and horizontal gallery navigation
 */
...
const THEMES_DIR = path.join(TEMPLATES_DIR, 'themes-glass');
const PREVIEW_DIR = path.join(ROOT_DIR, config.PATHS.preview);
const DATA_DIR = path.join(ROOT_DIR, config.PATHS.data);

const THEMES = ['frost', 'aurora'];

const THEME_NAMES = {
  frost: 'Ледена елеганција',
  aurora: 'Топла аурора'
};

const THEME_DESCRIPTIONS = {
  frost: 'Хладни плави тонови са леденим ефектом стакла',
  aurora: 'Топле румене нијансе са романтичним сјајем'
};

const THEME_FONTS = {
  frost: `<link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,500;1,400&family=Lato:wght@300;400&display=swap" rel="stylesheet">`,
  aurora: `<link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,500;1,400&family=Lato:wght@300;400&display=swap" rel="stylesheet">`
};
...
function replacePlaceholders(html, data) {
  const replacements = {
    ...
    'INVITATION_INTRO': data.invitation_intro || 'Позивамо вас',
...
function generateThemedPage(data, theme) {
  const baseTemplate = fs.readFileSync(path.join(TEMPLATES_DIR, 'base-glass.html'), 'utf8');
...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    :root{--primary:#B76E79;--primary-dark:#A05D67;--bg:linear-gradient(135deg,#E8D5C4 0%,#F5E6D8 100%);--white:rgba(255,255,255,0.9);--border:rgba(255,255,255,0.4);--muted:#666}
    body{font-family:'Inter',sans-serif;background:var(--bg);background-attachment:fixed;color:#3D3D3D;line-height:1.6;min-height:100vh}
    .container{max-width:900px;margin:0 auto;padding:0 1rem}
    .header{background:var(--white);backdrop-filter:blur(10px);border-bottom:1px solid var(--border);padding:1.5rem 0;position:sticky;top:0;z-index:100}
    .header-content{display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:1rem}
    .header-title{font-size:1.25rem;font-weight:600}
    .header-title span{color:var(--primary)}
    .expiry-timer{display:flex;align-items:center;gap:0.5rem;padding:0.5rem 1rem;background:rgba(183,110,121,0.1);border-radius:8px;font-size:0.875rem;color:var(--primary)}
    .main{padding:3rem 0}
    ...
    .badge{display:inline-block;background:var(--primary);color:#fff;padding:0.25rem 0.75rem;border-radius:50px;font-size:0.75rem;margin-bottom:1rem}
    .theme-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(350px,1fr));gap:2rem}
    .theme-card{background:var(--white);backdrop-filter:blur(10px);border-radius:16px;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,0.1);transition:transform 0.3s}
    .theme-card:hover{transform:translateY(-4px)}
    .theme-preview{aspect-ratio:3/4;overflow:hidden;background:linear-gradient(135deg,#E8D5C4,#F5E6D8)}
    ...
    .modal-overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,0.3);backdrop-filter:blur(5px);z-index:1000;align-items:center;justify-content:center}
    .modal-overlay.active{display:flex}
    .modal{background:var(--white);backdrop-filter:blur(20px);border-radius:16px;max-width:400px;padding:2rem;text-align:center}
...
<body>
  <header class="header"><div class="container"><div class="header-content">
    <h1 class="header-title">🖼️ <span>${data.bride_name} & ${data.groom_name}</span></h1>
  ...
  <main class="main"><div class="container">
    <div class="intro">
      <span class="badge">🖼️ Галерија</span>
      <h1>Изаберите тему позивнице</h1>
      <p>Галерија љубави — елегантни стаклени панели са хоризонталном навигацијом.</p>
    </div>
...
async function generatePreview(slug) {
  console.log(`\n🖼️ Generating glass gallery layout for: ${slug}\n`);

  const data = loadWeddingData(slug);
  data.slug = slug;

  const outputDir = path.join(PREVIEW_DIR, `${slug}-glass`);
  ...
  console.log(`\n✅ Done! URL: http://localhost:${config.DEV_SERVER_PORT}/preview/${slug}-glass/\n`);
}
...
module.exports = { generateGlassPreview: generatePreview };
