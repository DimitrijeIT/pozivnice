...
/**
 * Generate Telegram Layout Preview
 * Vintage 1920s typewriter style single-page invitation
 */
...
const THEME_FONTS = {
  western: `<link href="https://fonts.googleapis.com/css2?family=Special+Elite&family=Courier+Prime:ital,wght@0,400;0,700;1,400&display=swap" rel="stylesheet">`,
  express: `<link href="https://fonts.googleapis.com/css2?family=Special+Elite&family=Courier+Prime:ital,wght@0,400;0,700;1,400&display=swap" rel="stylesheet">`
};
...
function generateThemedPage(data, theme) {
  const baseTemplate = fs.readFileSync(path.join(TEMPLATES_DIR, 'base-telegram.html'), 'utf8');
...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    :root{--primary:#8B7355;--primary-dark:#6B5344;--bg:#2C2416;--paper:#F5E6C8;--border:#5C4A2A;--muted:#8B7355}
    body{font-family:'Inter',sans-serif;background:var(--bg);color:var(--paper);line-height:1.6;min-height:100vh}
    .container{max-width:900px;margin:0 auto;padding:0 1rem}
    .header{background:linear-gradient(90deg,var(--primary),#6B5344,var(--primary));padding:1.5rem 0;position:sticky;top:0;z-index:100}
    .header-content{display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:1rem}
    .header-title{font-size:1.25rem;font-weight:600}
    .expiry-timer{display:flex;align-items:center;gap:0.5rem;padding:0.5rem 1rem;background:rgba(245,230,200,0.2);border-radius:8px;font-size:0.875rem}
    .main{padding:3rem 0}
    ...
    .badge{display:inline-block;background:var(--paper);color:var(--bg);padding:0.25rem 0.75rem;border-radius:50px;font-size:0.75rem;margin-bottom:1rem}
    .theme-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(350px,1fr));gap:2rem}
    .theme-card{background:var(--paper);border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.3);transition:transform 0.3s}
    .theme-card:hover{transform:translateY(-4px)}
    .theme-preview{aspect-ratio:3/4;overflow:hidden;background:var(--bg)}
    .theme-info{padding:1.5rem;color:var(--bg)}
    .theme-name{font-size:1.25rem;font-weight:600;margin-bottom:0.5rem}
    .theme-description{color:var(--border);margin-bottom:1.25rem;font-size:0.9rem}
    ...
    .btn-primary{background:var(--primary);color:var(--paper);flex:1}
    ...
    .modal{background:var(--paper);color:var(--bg);border-radius:12px;max-width:400px;padding:2rem;text-align:center}
    .modal h2{margin-bottom:0.75rem}
    .modal p{color:var(--border);margin-bottom:1.5rem}
...
<body>
  <header class="header"><div class="container"><div class="header-content">
    <h1 class="header-title">📜 ${data.bride_name} & ${data.groom_name}</h1>
  ...
  <main class="main"><div class="container">
    <div class="intro">
      <span class="badge">📜 Телеграм</span>
      <h1>Изаберите тему позивнице</h1>
      <p>Хитна порука о вашем венчању — винтаж телеграм стил.</p>
    </div>
...
async function generatePreview(slug) {
  console.log(`\n📜 Generating telegram layout for: ${slug}\n`);

  const data = loadWeddingData(slug);
  data.slug = slug;

  const outputDir = path.join(PREVIEW_DIR, `${slug}-telegram`);
  ...
  console.log(`\n✅ Done! URL: http://localhost:${config.DEV_SERVER_PORT}/preview/${slug}-telegram/\n`);
}
...
module.exports = { generateTelegramPreview: generatePreview };
